...
⚠ Currently, Couchbase Lite 2.0 is available for Android only. The SDK
cannot be used in Java applications.
...
Open *MainActivity.java* in Android Studio and copy the following code in the `didStart` method. This snippet demonstrates how to run basic CRUD operations, a simple Query and running bi-directional replications with Sync Gateway.

[source,java]
----
include::{examplesdir}/DatabaseAPITest.java[tags=getting-started]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/[Java SDK
API References]
...
* Remove the existing Couchbase Lite dependency from the Android Studio
project.
* Install the Couchbase Lite 2.0 framework in your project (see the link:java.html#getting-started[installation options]). At this point, there will be many compiler warnings. Refer to the examples on this page to learn about the new API.
...
[source,java]
----
include::{examplesdir}/DatabaseAPITest.java[tags=1x-attachment]
----
...
[source,java]
----
include::{examplesdir}/DatabaseAPITest.java[tags=new-database]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/DatabaseConfiguration.html[`DatabaseConfiguration`]
object such as the database directory.

=== Finding a Database File

When the application is running on the Android emulator, you can locate
the application's data folder and access the database file by using the
*adb* CLI tools. For example, to list the different databases on the
emulator, you can run the following commands.
...
The *adb pull* command can be used to pull a specific database to your
host machine.
...
[source,java]
----
include::{examplesdir}/DatabaseAPITest.java[tags=logging]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/Database.html#copy-java.io.File-java.lang.String-com.couchbase.lite.DatabaseConfiguration-[`Database.copy(File path, String name, DatabaseConfiguration config)`]
method as shown below.

[source,java]
----
include::{examplesdir}/DatabaseAPITest.java[tags=prebuilt-database]
----
...
* The `database.getDocument(String id)` method can be used to get a
document. If it doesn't exist in the database, it will return `null`.
...
[source,java]
----
include::{examplesdir}/DocumentAPITest.java[tags=initializer]
----
...
[source,java]
----
include::{examplesdir}/DocumentAPITest.java[tags=update-document]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/Dictionary.html[`property accessors`]
...
[source,java]
----
include::{examplesdir}/DocumentAPITest.java[tags=date-getter]
----
...
the
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/Document.html#contains-java.lang.String-[`Document.Contains(String key)`]
method.
...
[source,java]
----
include::{examplesdir}/DocumentAPITest.java[tags=batch]
----
...
[source,java]
----
include::{examplesdir}/DocumentAPITest.java[tags=blob]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-select-meta]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-select-all]
----
...
The http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/Expression.html[comparison operators] can be used in the WHERE statement to specify on which
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-where]
----

==== Collection Operators

http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/ArrayFunction.html[Collection
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-collection-operator-contains]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-collection-operator-in]
----

==== Like Operator

The
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/com/couchbase/lite/Expression.html#like-com.couchbase.lite.Expression-[`like`]
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-like-operator]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-like-operator-wildcard-match]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-like-operator-wildcard-character-match]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-regex-operator]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-join]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-groupby]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-orderby]
----
...
[source,java]
----
include::{examplesdir}/QueryAPITest.java[tags=query-index]
----
...
[source,java]
----
include::{examplesdir}/FullTextSearchAPITest.java[tags=fts-index]
----
...
[source,java]
----
include::{examplesdir}/FullTextSearchAPITest.java[tags=fts-query]
----
...
Gateway
https://developer.couchbase.com/documentation/mobile/current/installation/sync-gateway/index.html[installation
guide].
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-java/db022/index.html?com/couchbase/lite/ReplicatorConfiguration.html[`ReplicatorConfiguration`]
...
[source,java]
----
include::{examplesdir}/ReplicationAPITest.java[tags=replication]
----
...
https://developer.couchbase.com/documentation/mobile/current/guides/sync-gateway/nginx/index.html[Load
Balancers]).
...
[source,java]
----
include::{examplesdir}/ReplicationAPITest.java[tags=replication-logging]
----
...
[source,java]
----
include::{examplesdir}/ReplicationAPITest.java[tags=replication-status]
----
...
[source,java]
----
include::{examplesdir}/ReplicationAPITest.java[tags=replication-error-handling]
----
...
[source,java]
----
include::{examplesdir}/ReplicationAPITest.java[tags=certificate-pinning]
----
...
* https://github.com/couchbase/couchbase-lite-android/issues/1264[*#1264*]
2.0 migration from Forest back to SQlite
* https://github.com/couchbase/couchbase-lite-android/issues/1577[*#1577*]
2.1: Eliminate CouchbaseLiteRuntimeExecption, convert it to
CouchbaseLiteExecption

*Bugs*

* https://github.com/couchbase/couchbase-lite-android/issues/1217[*#1217*]
2.0: Todo Sample bug Fix and implement remaining features
* https://github.com/couchbase/couchbase-lite-android/issues/1234[*#1234*]
2.0: Replicator continuous pull status completed and total number out of
sync
* https://github.com/couchbase/couchbase-lite-android/issues/1247[*#1247*]
2.0: Push Replication Causing Errors
* https://github.com/couchbase/couchbase-lite-android/issues/1273[*#1273*]
2.0: Replicator Activity Level stays in Busy state
* https://github.com/couchbase/couchbase-lite-android/issues/1347[*#1347*]
2.0: How do we properly stop puller Replicator and delete database ?
* https://github.com/couchbase/couchbase-lite-android/issues/1470[*#1470*]
2.0: In My App with CB Lite DB020(Android), ConflictResolver is called
endless loop during three device or much more!
* https://github.com/couchbase/couchbase-lite-android/issues/1499[*#1499*]
2.0: Repl Bad URL returns 19 instead of 15 on API 24 ARM emulator
* https://github.com/couchbase/couchbase-lite-android/issues/1500[*#1500*]
2.0: LoadTest.testUpdate() causes memory error with Android API 19 Arm
Emulator
* https://github.com/couchbase/couchbase-lite-android/issues/1501[*#1501*]
2.0: ReplicatorTest > testPushDoc fails
* https://github.com/couchbase/couchbase-lite-android/issues/1512[*#1512*]
2.0: ReplicatorTest > testPullDoc fails
* https://github.com/couchbase/couchbase-lite-android/issues/1581[*#1581*]
2.0 : LiveQuery threading issue
* https://github.com/couchbase/couchbase-lite-android/issues/1609[*#1609*]
2.0: testSelfSignedSSLPinned() unit test failure
* https://github.com/couchbase/couchbase-lite-android/issues/1610[*#1610*]
Exception from the cpp layer in my android app on load tests
* https://github.com/couchbase/couchbase-lite-android/issues/1615[*#1615*]
2.0: NullPointerException: Attempt to invoke virtual method 'boolean
com.couchbase.litecore.C4QueryEnumerator.next()' on a null object
reference
* https://github.com/couchbase/couchbase-lite-android/issues/1620[*#1620*]
NOT operator not working for FTS
* https://github.com/couchbase/couchbase-lite-android/issues/1630[*#1630*]
2.0: make public Index interface
* https://github.com/couchbase/couchbase-lite-android/issues/1651[*#1651*]
2.0: C4DocumentTest.testPut() fails
* https://github.com/couchbase/couchbase-lite-android/issues/1652[*#1652*]
App crash with deleteDocument with concurrencyControl
* https://github.com/couchbase/couchbase-lite-android/issues/1693[*#1693*]
DB023 - Crazy amount of dbListenerTokens - makes the DB run slow

*Known Issues*

* https://github.com/couchbase/couchbase-lite-android/issues/1685[*#1685*]
Replicator in Android get stuck at busy state when it goes back to
online from offline (Genymotion)
* https://github.com/couchbase/couchbase-lite-android/issues/1702[*#1702*]
ValueIndexItem.expression(Expression) is implemented with CBL Android 2.0.0 release.
